...
import { Link2Icon, TrashIcon, GripIcon, X, Plus, Minus, RotateCcw } from 'lucide-react';
import {
  getTypeMeta,
  getStatusMeta,
  getDisplayTypeLabel,
        ...
        <div className="min-h-0 flex-1 overflow-auto pr-1 no-scrollbar">
          {node.type === 'image' && node.imageSrc ? (
            <div className="flex min-h-0 flex-1 flex-col">
              <button
                type="button"
                className="group relative block min-h-[220px] flex-1 overflow-hidden rounded-md text-left"
                onClick={(event) => {
                  event.stopPropagation();
                  setImageScale(1);
                  setIsImageOpen(true);
