...
export function getNodeMinWidth(type) {
  return type === 'image' ? 320 : type === 'note' ? 240 : 180;
}

export function getNodeMinHeight(type) {
  return type === 'image' ? 260 : type === 'note' ? 130 : 110;
}
...
export function normalizeNode(node) {
  ...
  if (normalized.type === 'image' && normalized.imageSrc) {
    normalized.width = Math.max(normalized.width || 0, 420);
    normalized.height = Math.max(normalized.height || 0, 340);
  }
