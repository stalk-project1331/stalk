...
export default function Board() {
  ...
  const persistSession = useCallback(
    (
      ...
      try {
        ...
        touchProject(getActiveProjectId());
        renameUntitledProject(getActiveProjectId(), customNodes[0]?.title);
  ...
  useEffect(() => {
    const reloadProject = () => {
      const saved = localStorage.getItem(projectStorageKey(getActiveProjectId()));
      if (!saved) {
        setNodes([]);
        setEdges([]);
        setCanvasOffset({ x: 0, y: 0 });
      ...
      try {
        const parsed = JSON.parse(saved);
        setNodes(Array.isArray(parsed.nodes) ? parsed.nodes.map(ensureNodeEnrichment) : []);
        setEdges(Array.isArray(parsed.edges) ? parsed.edges : []);
        setCanvasOffset(parsed.canvasOffset || { x: 0, y: 0 });
        setCanvasScale(parsed.canvasScale || 1);
    ...
    window.addEventListener(PROJECT_CHANGED_EVENT, reloadProject);
    return () => window.removeEventListener(PROJECT_CHANGED_EVENT, reloadProject);
  }, []);
  ...
  const fitAllNodes = useCallback(() => {
    ...
    setShowZoomIndicator(true);
  }, [nodes, pulseViewportTransition]);

  useEffect(() => {
    const handleBoardCommand = (event) => {
      const { command, value } = event.detail || {};
      if (command === 'add-node' && value) handleAddNode(value);
      if (command === 'fit-all') fitAllNodes();
    };

    window.addEventListener('stalk-board-command', handleBoardCommand);
    return () => window.removeEventListener('stalk-board-command', handleBoardCommand);
  }, [fitAllNodes, handleAddNode]);
          ...
          prev.map((node) =>
            node.id === selectedNodeId
              ? {
                  ...
                  height: Math.max(node.height, 340),
